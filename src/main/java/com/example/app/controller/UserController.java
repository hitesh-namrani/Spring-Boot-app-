...
/*
REST controller that provides API endpoints for user account operations
...
public class UserController {
    ...
    UserController(UserService service, JwtUtil jwtUtil) {
    ...
    @param request contains the username and password for the new user
    @return an API response containing the newly created user
    ...
    */
    ...
    public ApiResponse register(@RequestBody AuthRequest request) throws WalletException {
        final User newUser = service.registerUser(request.getUsername(), request.getPassword());
        return new ApiResponse(Status.SUCCESS, "Account created successfully", newUser);
    }

    /*
    Authenticates a user and initializes a secure session.

    @param request contains the user's login credentials
    @return API response containing the JWT token and authenticated user details
    ...
    */
    ...
    public ApiResponse login(@RequestBody AuthRequest request) throws WalletException {
        User user = service.verifyLogin(request.getUsername(), request.getPassword());

        String token = jwtUtil.generateToken(user.getUsername());
        Map<String, Object> data = new HashMap<>();
        data.put("token", token);

        return new ApiResponse(Status.SUCCESS, "Logged in successfully", data, user);
    }

    /*
    Since authentication is stateless using JWT, the server does not invalidate the token.
    The user is responsible for deleting the stored token.
    @return An API response indicating successful logout status
    */
    @PostMapping("/logout")
    public ApiResponse logout() {
        return new ApiResponse(Status.SUCCESS, "Logged out successfully (Delete token on user side)", null);
    }

    /*
    Processes a financial transaction (deposit or withdrawal) for the authenticated user.
    ...
    @return an API response containing the transaction status and updated user details
    ...
    */
    ...
    /*
    Retrieves the current wallet balances and transaction limits for the authenticated user.
    @param authHeader JWT authorization header
    @return an API response containing the user's current balance
    ...
    */
    ...
    public ApiResponse getBalance(@RequestHeader(value = "Authorization", required = false) String authHeader) throws WalletException {
        String sessionUser = jwtUtil.validateHeaderAndExtractUsername(authHeader);
        final User user = service.getUserByUsername(sessionUser);
        Map<String, Object> data = service.limits(sessionUser);
        return new ApiResponse(Status.SUCCESS, "Balance retrieved successfully", data, user);
    }
    ...
    @param authHeader JWT authorization header
    @return API response containing the user's transaction history
    @throws WalletException if authentication fails
    */
    @GetMapping("/history")
    public ApiResponse getHistory(@RequestHeader(value = "Authorization", required = false) String authHeader) throws WalletException {
        ...
        return new ApiResponse(Status.SUCCESS, "Transaction history retrieved successfully", responseData, user);
    }
}